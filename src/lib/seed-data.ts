// Shared demo-data seeding logic for BroadcastHub.
//
// Used by:
//   - `scripts/seed.ts` (run via `bun run db:seed`)
//   - The admin "Reset demo data" API route
//
// Both call `seedDemoData(db)` (or `resetDemoData(db)`) with a PrismaClient
// they own; this module never creates or disconnects the client itself.
...
/**
 * Populate the demo BroadcastHub database. Idempotent on the org + users +
 * AI settings (uses upsert); other entities are created fresh each call.
 *
 * Does NOT call `db.$disconnect()` — the caller owns the PrismaClient.
 */
...
- If an order is delayed beyond the estimated window, customers receive a 10% credit on their next purchase.`,
  ...
  return {
    ...
    completedCampaignId: completedCampaign.id,
  };
}

/**
 * Wipe the default BroadcastHub org (cascade deletes all related data) and
 * re-seed fresh demo data. Safe to call even if the org doesn't exist yet.
 *
 * Does NOT call `db.$disconnect()` — the caller owns the PrismaClient.
 */
export async function resetDemoData(db: PrismaClient): Promise<SeedResult> {
  const existing = await db.organization.findUnique({
    where: { slug: "broadcasthub" },
    select: { id: true },
  });
  if (existing) {
    // Schema has `onDelete: Cascade` on every org-owned relation, so this
    // single delete cascades to contacts, campaigns, conversations, KB docs,
    // chunks, AI settings, social posts, automations, webhooks, etc.
